...
```{r}
experiments <- labguru_list_experiments(folder_id  = 1,
                                        get_cols   = "limited")
experiments
```
...
```{r}
experiment_1 <- labguru_get_experiment(experiment_id = 1)
experiment_1
```
...
```{r}
experiment_new <- labguru_add_experiment(title       = "My new experiment",
                                         project_id  = 91,
                                         folder_id   = 41,
                                         description = "This experiment is a test from LabguruR",
                                         return      = "all")
experiment_new[1:4]
```
...
```{r}
ep_list <- labguru_list_experiment_procedures(experiment_id = 91, get_cols = "all")
dplyr::glimpse(ep_list)
```

```{r}
ep_new <- labguru_add_experiment_procedure(name          = "New procedure",
                                           experiment_id = 91)
ep_new
```
...
```{r}
ep_431 <- labguru_get_experiment_procedure(experiment_procedure_id = 431)
ep_431
```
...
```{r}
el_list <- labguru_list_elements(experiment_id = 91)
```

```{r}
el_new <- labguru_add_element(data = "<p>Hello World</p>",
                              experiment_procedure_id = 431)
el_new
```


```{r}
el_901 <- labguru_get_element(element_id = 901)
el_901
```
